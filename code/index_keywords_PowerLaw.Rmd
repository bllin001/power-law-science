---
title: "Power Law: Social Resilience Index Keywords" 
...
```{r}
# Data Lecture
data <- read.csv("../data/index_keyword_frequency.csv")
head(data)
...
```{r}
library(openxlsx) 
write.xlsx(df.word_count, "index_keywords_word_count.xlsx", 
...
ggplot(df.word_count, aes(x = rank, y = Total)) + 
  ...
  theme_bw() + 
  labs(x = "Rank", y = "Count", title = "Ranking Distribution for index keywords") +
...
library(openxlsx) 
write.xlsx(Estimation, "index_keywords_Estimation.xlsx", 
           sheetName = "results", rowNames = FALSE)
write.csv(Estimation,file="index_keywords_Estimation.csv") 
